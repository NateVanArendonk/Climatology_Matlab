function out = test_getter(usaf,wban,dates,station_name)
...
%data.
    ...
    for x = 0:date_dif
        ...
        %load up the contents of the directory and see if the date for the
        %station exists
        temp_dir = dir(data_source);
        temp = struct2cell(temp_dir).'; 
        name = temp(:, 1); 
        clear temp temp_dir;
        %now see if the file exists, if it does, get it, if not, carry on
        if ismember(file_name, name) == 1;
            ...
            file_out = strcat(folder_nm,under_sc,date_str,'.csv');
            ncdc_parse_working(file_use,file_out)
        %elseif ismember(file_name, name) == 0;
        %    continue
        %else
