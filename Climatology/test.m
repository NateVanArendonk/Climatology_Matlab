...
%% Find yearly max
yr_vec = year(station_data.time(2)):year(station_data.time(end)); %make a year vec
total_months = months(station_data.time(1),station_data.time(end));
%make a monthly time vector
mo_vec = datenum(year(station_data.time(1)),month(station_data.time(1)),15):(365.25/12):datenum(year(station_data.time(end)),month(station_data.time(end)),15);


maxima = NaN(length(mo_vec),1); %create vector to house all of the block maxima


for j = 1:length(mo_vec)
    inds = find(year(station_data.time) == year(mo_vec(j)) & month(station_data.time) == month(mo_vec(j)));
    if length(inds) < 730/2
        maxima(j) = NaN;
    else
        maxima(j) = max(station_data.wndspd(inds));
    end
end
...
nan_ind = find(maxima == 999); % Find any nans and get rid of them
maxima(nan_ind) = [];
...
%%
% Save the Plot
cd('../../Matlab_Figures/GEV/Raw_data')
